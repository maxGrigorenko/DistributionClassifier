...
def create_characteristics(knn_graph_exp, distance_graph_exp, knn_graph_pareto, distance_graph_pareto, distrib_type=None):
    if distrib_type == "knn":
...
def get_characteristics(lambda_param, alpha_param, n, k, d, distrib_type=None):
    ...
    return create_characteristics(knn_graph_exp, distance_graph_exp, knn_graph_pareto, distance_graph_pareto)

def get_average_characteristics(lambda_param, alpha_param, n, k, d, distrib_type=None):
    characteristics_list = []
    for trial in range(5):
        characteristics = get_characteristics(lambda_param, alpha_param, n, k, d, distrib_type)
        characteristics_list.append(characteristics)

    r = None
    if distrib_type == "knn":
        r = Characteristics(
            knn_exp_components=np.mean([data.knn_exp_components for data in characteristics_list]),
            knn_pareto_components=np.mean([data.knn_pareto_components for data in characteristics_list]),
            knn_exp_chromatic=np.mean([data.knn_exp_chromatic for data in characteristics_list]),
            knn_pareto_chromatic=np.mean([data.knn_pareto_chromatic for data in characteristics_list]),
        ...
        r = Characteristics(
            ...
            distance_exp_chromatic=np.mean([data.distance_exp_chromatic for data in characteristics_list]),
            distance_pareto_chromatic=np.mean([data.distance_pareto_chromatic for data in characteristics_list]),
            distance_exp_components=np.mean([data.distance_exp_components for data in characteristics_list]),
            distance_pareto_components=np.mean([data.distance_pareto_components for data in characteristics_list]),
        )
